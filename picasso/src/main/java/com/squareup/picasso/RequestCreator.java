...
public class RequestCreator {
  ...
  RequestCreator(Picasso picasso, Uri uri, int resourceId) {
    ...
    this.data = new Request.Builder(uri, resourceId, picasso.defaultBitmapConfig);//TODO 根据2.5.2 修改
  }

   RequestCreator() {
    this.picasso = null;
    this.data = new Request.Builder(null, 0, null);//TODO 根据2.5.2 修改
  }
  ...
  public void fetch(Callback callback) {
    ...
    if (data.hasImage()) {
      // Fetch requests have lower priority by default.
      if (!data.hasPriority()) {
        data.priority(Priority.NORMAL);//TODO 此处默认为Low,改为正常
      }
